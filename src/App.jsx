...
export default function PedidoRapido() {
  ...
  const loadTenantFromURL = useCallback(async (slug) => {
    ...
    try {
      ...
      if (tenant) {
        ...
        let prods = [];
        try {
          prods = await apiFetch('products', { eq: { column: 'tenant_id', value: tenant.id } });
        } catch (fetchError) {
  ...
  const createTenant = async (name, slug, whatsapp, password) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to fetch products for new tenant:', fetchError);
      }
      setProducts(prods);
      window.history.pushState({}, '', `/${slug}`);
      setView('menu');
  ...
  const CartPage = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <header className="bg-gr...(truncated 7366 characters)...-40">
          <div className="max-w-4xl mx-auto flex justify-between items-center">
            <div>
